...
import { LocationService } from '../../../Services/SetUp/location/location.service';
...
// --------gridOninit---------------
 ...
 this.columnDefs = [
  ...
  { headerName: 'LocationName	', field: 'locationName', filter: 'agTextColumnFilter', width:400 }  // Text filter for Name
];
...
console.log('Data saved successfully!',formData )
      ...
      this.LocationService. saveLocation(formData).subscribe(
        (response) => {
          ...
          this.formGroup.get('LocationName')?.disable(); // Disable the Custgroupname field
          this.isSaving = true;
  ...
  onSearch() { 
    ...
    this.LocationService.getRootMenus().subscribe(
      (response: any) => {
  ...
  this.formGroup.get('LocationName')?.enable();
this.isEdit = true
  ...
  onRowDoubleClicked(event: any): void {
    ...
    this.LocationService.getLocationById(rowCode).subscribe(
      (details) => {
        ...
        this.formGroup.get('Id')?.disable(); // Disable the Code field
          ...
          this.formGroup.patchValue({
            ...
            LocationName: details.obj.locationName, // Assuming the response contains this structure
          });
          this.isSaving = true;
