...
import { CompanyService } from '../../../Services/SetUp/company/company.service';
...
// --------gridOninit---------------
 ...
 this.columnDefs = [
  ...
  { headerName: 'CompanyName	', field: 'companyName', filter: 'agTextColumnFilter', width:400 }  // Text filter for Name
];
    ...
    if (this.formGroup.valid) {
      ...
      this.CompanyService.saveCompany(formData).subscribe(
        (response) => {
          ...
          this.formGroup.get('CompanyName')?.disable(); // Disable the Custgroupname field
          this.isSaving = true;
  ...
  onSearch() { 
    ...
    this.CompanyService.getRootMenus().subscribe(
      (response: any) => {
...
this.isEdit = true
  ...
  onRowDoubleClicked(event: any): void {
    ...
    this.CompanyService.getCompanyById(rowCode).subscribe(
      (details) => {
        ...
        this.formGroup.get('Id')?.disable(); // Disable the Code field
          ...
          this.formGroup.patchValue({
            ...
            CompanyName: details.obj.companyName, // Assuming the response contains this structure
          });
          this.isSaving = true;
